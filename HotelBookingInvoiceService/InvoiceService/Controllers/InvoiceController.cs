...
namespace InvoiceService.Controllers
{
    ...
    public class InvoiceController : ControllerBase
    {
        ...
        // Download invoice PDF only (no email)
        [HttpPost("download-only")]
        public IActionResult DownloadInvoice([FromBody] InvoiceRequest request)
        {
            try
            {
                Console.WriteLine($"[DEBUG] Generating PDF for download only: {request.BookingReference}");
                
                var pdf = GenerateInvoicePdf(request);
        ...
        // Send invoice via email only (no download)
        [HttpPost("send-email")]
        public async Task<IActionResult> SendInvoiceEmail([FromBody] InvoiceRequest request)
        {
            try
            {
                Console.WriteLine($"[DEBUG] Sending invoice email to: {request.GuestEmail}");
                
                if (string.IsNullOrEmpty(request.GuestEmail))
                {
                    return BadRequest(new { error = "Guest email is required" });
                }

                var pdf = GenerateInvoicePdf(request);
                await _emailService.SendInvoiceEmailAsync(request.GuestEmail, request.GuestName, pdf, request.BookingReference);
                
                return Ok(new { message = "Invoice sent successfully to " + request.GuestEmail });
            }
            catch (Exception ex)
            {
                return BadRequest(new { error = ex.Message });
        ...
        // Generate invoice: both download AND email (backward compatibility)
        [HttpPost("generate")]
        public async Task<IActionResult> GenerateInvoice([FromBody] InvoiceRequest request)
        {
            try
            {
                Console.WriteLine($"[DEBUG] Generating invoice (download + email): {request.BookingReference}");
        ...
        // Helper method to generate invoice PDF
